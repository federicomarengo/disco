...
export default function MatchOverlay({ onClose, profile }: MatchOverlayProps) {
  ...
  return (
    <motion.div
      ...
      style={{ background: 'linear-gradient(160deg, rgba(244,63,94,0.88) 0%, rgba(168,85,247,0.88) 100%)' }}
    >
      {[...Array(12)].map((_, i) => (
        <motion.div
          key={i}
          ...
          animate={{ x: (Math.random() - 0.5) * 300, y: (Math.random() - 0.5) * 600, opacity: [0, 1, 0] }}
      ...
      >
        ...
        <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.5 }}
          className="space-y-2">
          <p className="text-white text-xl font-semibold">Vos y {profile.name} se gustaron</p>
          <p className="text-white/70 text-sm">Están esta noche en el mismo lugar 🎉</p>
        </motion.div>
